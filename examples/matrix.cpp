...
template <size_t size> void static_matrix_test() {
  ...
  std::cout << std::endl
            << "---------------------------------" << std::endl
            << "Static test: size = " << size << std::endl
            << "---------------------------------" << std::endl
            << std::endl;
  ...
  std::cout << "mat.transpose()[0] = " << mat.transpose()[0] << std::endl
            << std::endl;

  std::cout << "vec = \n" << vec << std::endl << std::endl;
  std::cout << "vec.transpose() = " << vec.transpose() << std::endl
            << std::endl;
  ...
  std::cout << "mat.transpose() * mat = \n"
            ...
            << std::endl;
  std::cout << "vec.transpose() * mat = " << vec.transpose() * mat << std::endl
            << std::endl;
  std::cout << "vec * vec.transpose() = \n"
            << vec * vec.transpose() << std::endl
            << std::endl;

  std::cout << "vec.transpose() * vec = " << vec.transpose() * vec << std::endl
            << std::endl;

  float scalar = (vec.transpose() * vec)[0][0];
  std::cout << "float(vec.transpose() * vec) = " << scalar << std::endl
            << std::endl;
  ...
  std::cout << "mat * mat.inverse() > 0.000001 = \n"
            << ((mat * mat.inverse()) > 0.000001) << std::endl
            << std::endl;

  mat.slice(StaticIndexTuple<size / 2, 0>(), vec.transpose().shape()) =
      vec.transpose();
  std::cout << "mat.slice(StaticIndexTuple<size/2,0>(), "
               "vec.transpose().shape() ) = vec.transpose() = \n"
            << mat << std::endl
            << std::endl;
}
...
void dynamic_matrix_test(int size) {
  std::cout << std::endl
            << "---------------------------------" << std::endl
            << "Dynamic test: size = " << size << std::endl
            << "---------------------------------" << std::endl
            << std::endl;
  ...
  vec.resize(size, 1);
  vec.for_all_indices([&](decltype(vec)::Index idx) {
    int i = idx.get<0>();
    vec(idx) = i + 1;
  });
  ...
  mat.slice(DynamicIndexTuple<2>(size / 2, 0), vec.transpose().shape()) =
      vec.transpose();
  std::cout << "mat.slice(DynamicIndexTuple<2>(size/2,0), "
               "vec.transpose().shape() ) = vec.transpose() = \n"
            << mat << std::endl
            << std::endl;
}
