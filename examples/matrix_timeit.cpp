...
template <size_t N>
using matrixh =
    MatrixCreator<HeapNDArray>::NewNDArray<float, StaticIndexTuple<N, N>>;
template <size_t N>
using vectorh =
    MatrixCreator<HeapNDArray>::NewNDArray<float, StaticIndexTuple<N, 1>>;
template <size_t N>
using matrixs =
    MatrixCreator<StackNDArray>::NewNDArray<float, StaticIndexTuple<N, N>>;
template <size_t N>
using vectors =
    MatrixCreator<StackNDArray>::NewNDArray<float, StaticIndexTuple<N, 1>>;

template <class M>
typename M::Scalar __attribute__((noinline)) determinant(const M &m) {
  return m.determinant();
}

template <class M, class V>
typename V::Copy __attribute__((noinline))
multiplication(const M &m, const V &v) {
  return m * m * v;
}

template <class M>
typename M::Copy __attribute__((noinline)) inversion(const M &m) {
  return m.inverse();
}
...
template <class M, class V> void measure(std::string type) {
  ...
  vec.for_all_indices([&](typename decltype(mat)::Index idx) {
    int i = idx.template get<0>();
    vec(idx) = i + 1;
  });

  std::cout << (type + " multiplication ")
            << time_it([&]() { return multiplication(mat, vec); }, 1000)
            << std::endl;
  std::cout << (type + " determinant ")
            << time_it([&]() { return determinant(mat); }, 1000) << std::endl;
  std::cout << (type + " inversion ")
            << time_it([&]() { return inversion(mat); }, 1000) << std::endl;
}
